...
export class JunggoService {
  ...
  async modify_junggo(
    ...
    try {
      ...
      // 2. 작성자와 요청자가 일치하는지 확인
      if (junggo.nickname !== userid) {
        throw new HttpException(`물품 수정 권한이 없습니다.`, HttpStatus.FORBIDDEN);
      }

      // 3. 이미지 교체 (새 이미지가 들어왔으면)
      let newImageUrl = junggo.img_url;
      ...
      if (file) {
        ...
        if (junggo.img_url) {
          try {
            ...
            throw new HttpException(`기존 이미지 삭제 실패: ${err.message}`, HttpStatus.BAD_REQUEST);
          }
        }

        // 새 이미지 업로드
        try {
          newImageUrl = await this.gcsConfigure.uploadFile(file);
        } catch (err) {
          throw new HttpException(`새 이미지 업로드 실패: ${err.message}`, HttpStatus.BAD_REQUEST);
        }
      }

      // 4. DB 업데이트 (물품 정보 수정)
      await this.junggoRepository.update(product_id, {
        token: modifyJunggo.token,
        product: modifyJunggo.product,
        product_status: modifyJunggo.product_status,
        method: modifyJunggo.method,
        delivery_fee: modifyJunggo.delivery_fee,
        trading_area: modifyJunggo.trading_area,
        product_description: modifyJunggo.product_description,
        img_url: newImageUrl,
      });

      return {
        message: '중고 물품이 성공적으로 수정되었습니다.',
      };
    } catch (error) {
      if (error instanceof HttpException) {
        throw error;
      }
      throw new HttpException(`중고 물품 수정 실패: ${error.message}`, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
